...
> **Об адресации.** Достоверная модель прямого доступа к VRAM 320×256 — окно `#50` + `PORT_Y`
> + базы `#C000`/`#C140` (см. [Пример 0](#пример-0-smoke-test-cpu-view-проверка-адресации-320256)
> и [5.3](03_mode1_320.md)). Некоторые примеры ниже, помеченные «упрощённо», иллюстрируют
> **логику алгоритма** (линия, прокрутка, анимация) с упрощённой адресацией `BASE + y*320 + x`
> — её перед запуском нужно заменить на корректную (`PORT_Y` на строку + `#C000 + x`).

---

## Пример 0: smoke-test CPU-view (проверка адресации 320×256)

Минимальный тест, который подтверждает всю цепочку прямого доступа: оба буфера, палитра,
`PORT_Y`, базы `#C000`/`#C140`, flip. Если изображение корректно (нет заворота на ~64 px,
оба буфера видны) — адресация настроена верно. Это рекомендуемая «первая программа» при
освоении графики.

```asm
; 1) Режим 320x256x8 на обоих экранах.
        ld      a, #81
        ld      b, #01
        ld      c, #50
        rst     #10
...
; 2) Одинаковая палитра в обе страницы: цвет 1 = белый (R=G=B=255).
        ld      a, #50
        out     (#E2), a            ; WIN3 = #50
        ld      a, #01
        out     (#89), a            ; PORT_Y = индекс цвета 1
        ld      a, #FF
        ld      (#C3E0), a          ; палитра 0: R
        ld      (#C3E1), a          ; G
        ld      (#C3E2), a          ; B
        ld      (#C3E4), a          ; палитра 1: R
        ld      (#C3E5), a          ; G
        ld      (#C3E6), a          ; B

; 3) В строке 100 буфер 0 — полоса цвета 1 через всю ширину (0..319).
        ld      a, 100
        out     (#89), a
        ld      hl, #C000
        ld      (hl), #01
...
; 4) В той же строке буфер 1 — полоса со сдвигом (x = 64..263).
        ld      hl, #C140 + 64
        ld      (hl), #01
        ld      de, #C140 + 65
        ld      bc, 199
...
; 5) Переключать отображаемый буфер по клавише (RGMOD бит 0).
.flip:
        in      a, (#C9)
        xor     #01
        out     (#C9), a
...
**Что проверяет:** в буфере 0 полоса идёт через всю строку (0..319) — значит база `#C000`
и 16-битный X верны; в буфере 1 полоса смещена (база `#C140`); при flip видны оба буфера
без заворота на 64 px. Несовпадение указывает на типичные ошибки из [5.3, «Подводные
камни»](03_mode1_320.md).
...
; Установить режим 320×256, нарисовать пиксели, ждать клавишу, выйти
        ...
        org     #8100
main:
        ; Установить режим 320×256 на ОБОИХ экранах (код #81, не «1»!)
        ld      a, #81
        ld      b, 1
        ld      c, #50              ; Dss.SetVMod
        rst     #10
        ...
        ld      hl, 1
        ld      e, 255 : ld d, 255 : ld a, 255
        ld      c, #A4              ; Bios.SetPalette
        rst     #08

        ; Подключить страницу #50 в WIN3 и выбрать строку растра
        ld      a, #50
        out     (#E2), a
        ld      a, 100
        out     (#89), a            ; PORT_Y = строка 100

        ; Нарисовать 100 белых пикселей в строке 100 буфера 0 (база #C000)
        ld      hl, #C000
        ld      b, 100
.loop:
        ld      (hl), 1
...
```asm
        ...
        ; Палитра
        ld      hl, 1
        ld      e, 255 : ld d, 0 : ld a, 0   ; красный
        ld      c, #A4 : rst #08
...
.fade_loop:
        ...
        ; Установить 16 основных цветов с масштабом B/255
        ld      c, 0                ; индекс цвета
.color_loop:
        push    bc
        push    bc

        ; R = base_R * B / 255 (упрощение)
        ld      h, 0
        ld      l, c
