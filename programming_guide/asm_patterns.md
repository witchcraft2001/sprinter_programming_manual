...
- BIOS и DSS ожидают, что стек лежит в окне `#8000-#BFFF`. Если ваша программа переносит SP выше (например, в окно с VRAM), перед `RST 08h/10h` обязательно переключайтесь на «системный» стек — иначе ОС затрёт ваш буфер [docs/bios.doc:1-40] [docs/Sprinter_Programming.pdf:680-720].
...
`espprobe` содержит готовые процедуры `open_isa_ports`, `reset_isa`, `close_isa_ports`. С учётом схемы из «Programming ISA» минимальный шаблон выглядит так:

1. Сохранить `PAGE3` и записать `0x10/0x11` в системный порт `#1FFD`, чтобы ПЛИС передала шину ISA устройству.
2. Записать в `PAGE3/#E2` байт `0xD0..0xD7`: `bit2` выбирает слот, `bit1` — память/порты.
3. Через `Port.ISA=9FBDh` поднять/опустить `RESET` (бит7), `AEN` (бит6) и адресные линии `A19..A14` (биты5..0).
4. Работать с устройством как с памятью в окне `#C000-#FFFF` (для портов — просто `LD (0Cxxx),A`).

После выхода обязательно вернуть исходную страницу и карту (`OUT (PAGE3),save_mmu3`), иначе верхнее окно останется привязанным к ISA [docs/Programming ISA.odt:1-90] [docs/Sprinter_Programming.pdf:1135-1235].
