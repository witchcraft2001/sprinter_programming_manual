...
## Загрузка системного сектора

При cold boot BIOS не ищет PC-совместимый boot sector с сигнатурой `55AA`.
Для Sprinter системным считается сектор собственного формата, который BIOS
читает с выбранного стартового устройства и проверяет по заголовку:

```asm
SYSID   DB "Starting...",0
```

Если сигнатура не совпала, BIOS считает сектор не системным и переходит к
ошибке загрузки или альтернативному устройству.

### Что именно читает BIOS

В `OS_LOAD` BIOS вызывает `DRV_READ` для чтения одного сектора:
...
На уровне контракта это означает:

- `A` — идентификатор стартового устройства;
- `B = 1` — читается один сектор;
- `HL = 0`, `IX = 1` — первый системный сектор;
- `DE = #7E00` — временный буфер.

### Куда передаётся управление

После успешного чтения BIOS:

1. проверяет заголовок `Starting...,0`;
2. переносит считанный сектор из `#7E00` в рабочую область;
3. устанавливает стартовое окружение загрузчика;
4. передаёт управление коду сектора.

Ключевой trampoline в BIOS:
...
Первые 12 байт сектора заняты сигнатурой `Starting...,0`, поэтому исполняемый
код стартует с `#800C`.

### Окружение загрузчика при входе

В момент передачи управления системный сектор получает:

- `PC = #800C`;
- `SP = #7FFF`;
- `IM 1`, `DI`;
- по адресу `#8000..#81FF` расположен считанный сектор;
- в `A` передаётся идентификатор стартового устройства.

Типовые диапазоны идентификатора устройства:
...
После этого BIOS boot path заканчивается, и дальнейшая инициализация уже
относится к загрузчику DSS.
